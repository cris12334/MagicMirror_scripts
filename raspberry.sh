...
if [ $doInstall == 1 ]; then
	...
	if git clone --depth=1 https://github.com/cris12334/MagicMirror.git; then
		echo -e "\e[92mCloning MagicMirror Done!\e[90m" | tee -a $logfile
		# replace faulty run-start.sh
		curl -sL https://raw.githubusercontent.com/cris12334/MagicMirror_scripts/master/run-start.sh >MagicMirror/run-start.sh
	...
	if [ $(grep version package.json | awk -F: '{print $2}') == '"2.9.0",' -a $ARM == 'armv6l' ]; then
	    git fetch https://github.com/cris12334/MagicMirror.git develop >/dev/null 2>&1
	...
	if verlte "2.11.0" $newver; then
	  ...
	  if [ "$ARM" == "armv6l" ]; then	
		  ...
		  curl -sL https://raw.githubusercontent.com/cris12334/MagicMirror_scripts/master/run-start.sh >run-start.sh
...
if [[ $choice =~ ^[Yy]$ ]]; then
      echo install and setup pm2 | tee -a $logfile
		...
		if [ ! -e installers/pm2_MagicMirror.json ]; then 
			curl -sL https://raw.githubusercontent.com/cris12334/MagicMirror_scripts/master/pm2_MagicMirror.json >installers/pm2_MagicMirror.json
			curl -sl https://raw.githubusercontent.com/cris12334/MagicMirror_scripts/master/mm.sh >installers/mm.sh
