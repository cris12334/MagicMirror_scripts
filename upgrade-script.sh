...
if [ -d ~/MagicMirror ]; then
  ...
  echo the log will be $logfile
    ...
    #   echo not english or locale not set, set git alias >>$logfile
		...
		remote=$(git remote -v 2>/dev/null | grep -i cris12334 | grep fetch | awk '{print $1}')
		...
		if [ "$remote." != "." ]; then
		  ...
		  # get the local and remote package.json versions
			...
			remote_version=$(curl -s https://raw.githubusercontent.com/cris12334/MagicMirror/master/package.json | grep -m1 version | awk -F\" '{print $4}')
										...
										justloaded=false
											# if we want just the modules listed in config.js now
												 ...
												 if [ ! -f ~/MagicMirror/installers/dumpactivemodules.js ]; then
														...
														curl -sL https://raw.githubusercontent.com/cris12334/MagicMirror_scripts/master/dumpactivemodules.js> ~/MagicMirror/installers/dumpactivemodules.js
														justloaded=true
												 fi
